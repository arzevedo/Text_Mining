...
```
Here we see the most worsds in each episode. I've filtered the stop words and character names because Legasov appears just too much in every episode besides the first one.
After that it was holesome see the major topics appers. Remember the _spoiler_ embargo for this miniseries ins't over yet.  So we can say that the first episode is all about a building or reactor that may or may not be on fire. The second also has fire but has water too and soldiers uses radio.
Well you got the ideia. Next we'll see what the different episodes have of different among them.


## TF-IDF ☢️
The idea do tf-idf is to find the important words for the content of each episode by decreasing the weight for the commonly used words and increasing the weight for words that are not used very much.  
```{r TF-IDF, fig.align="center", fig.height=9}
...
chernobyl_tidy_fil_per_epi %>%
  ...
  theme(panel.background = element_rect(fill = "#F2E205"), strip.text = element_text(size = 13),
        ...
        plot.background = element_rect(fill = "#F2E205", color = "#F2E205"))
```
This time we don't filter the most frequent words because their weight is important to see the most important words in each episode.

```{r Relationship between words, fig.height=8, fig.width=10, fig.align="center"}
...
## Sentiment ☢️
Now let1s perform a sentiment analysis on the bigram data, we can examine how often sentiment-associated words are preceded by _not_ or other negative words. We could use this to ignore or even reverse their contribution to the sentiment score.
ON A HOLD
```{r wrong sentiment}
